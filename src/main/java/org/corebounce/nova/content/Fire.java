...
public class Fire extends Content {
  ...
  static final int N_SPHERES = 8;
  static final float JITTER = 0.5f;
  ...
  public Fire(int dimI, int dimJ, int dimK) {
    super("Fire", dimI, dimJ, dimK);
  ...
  public void fillFrame(float[] rgbFrame, double timeInSec) {
    ...
    for (int i = 0; i < N_SPHERES; i++) {
      sphere(
        ...
        spheres[i * 5 + 1],
        spheres[i * 5 + 2],
        spheres[i * 5 + 3] * (0.5f + ((spheres[i * 5 + 2] - dimKf) / (-2f * dimKf)))
      );
    ...
    sphere(
      rgbFrame,
      (dimI - JITTER) / 2f + rnd.nextFloat() * JITTER,
      (dimJ - JITTER) / 2f + rnd.nextFloat() * JITTER,
      0,
      (dimI + dimJ) / 3f
    );
  }

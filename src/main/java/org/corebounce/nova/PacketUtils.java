...
public final class PacketUtils implements IConstants {
  ...
  static final int FLASH_OVERRIDE = 0x80;
  static final int INTERNETWORK_CONTROL = 0xC0;
  static final int NETWORK_CONTROL = 0xE0;
  ...
  public static void UDP(byte[] packet, int off, byte[] src, int srcPort, byte[] dst, int dstPort, int payloadLength) {
    ...
    packet[off++] = (byte) (LOW_DELAY | HIGH_RELIABILITY | CRITIC_ECP);
    ...
    packet[off++] = 0;
    packet[off++] = 0;
    System.arraycopy(src, 0, packet, off, 4);
    off += 4;
    System.arraycopy(dst, 0, packet, off, 4);
    off += 4;
  ...
  public static void chainData(
    ...
    for (int i = 0; i < 10; i++) {
      int pdata = pixelData[pixelDataOff + i];
  ...
  private static final int[] OP_MODE_BUF = new int[25 * 10];

...
public class SockAddr {
  ...
  public static final class InetSockAddr extends SockAddr {
    ...
    @Override
    public String toString() {
      return "AF_INET" + "[" + PcapUtils.toAddressString(address()) + (port == 0 ? "" : "port=" + port) + "]";
    }
  }
  ...
  public static final class PacketSockAddr extends SockAddr {
    ...
    PacketSockAddr(MemorySegment addr, Arena arena) {
      ...
      // patching this for JDK 23, see: https://github.com/slytechs-repos/jnetpcap-wrapper/pull/57
  ...
  public static final class LinkSockAddr extends SockAddr {
    ...
    LinkSockAddr(MemorySegment addr, Arena arena, int totalLength) {
      ...
      this.address = (alen == 0) ? Optional.empty() : Optional.of(saSegment.asSlice(off, alen).toArray(JAVA_BYTE));
      off += alen;
  ...
  public static final class IrdaSockAddr extends SockAddr {
    ...
    IrdaSockAddr(MemorySegment addr, Arena arena) {
      super(
        addr.reinterpret(SOCK_ADDR_IRDA_LEN, arena, EMPTY_CLEANUP),
        SockAddrFamily.IRDA,
        OptionalInt.of(SOCK_ADDR_IRDA_LEN)
      );
    ...
    public String toString() {
      return (
        "AF_IRDA [" +
        "id=" +
        PcapUtils.toAddressString(irdaDeviceID) +
  ...
  static <T extends SockAddr> T newInstance(Object value, Arena arena) {
    ...
    if (ForeignUtils.isNullAddress(addr)) return null;
  ...
  private static int readFamilyField(MemorySegment mseg) {
    if (NativeABI.isBsdAbi()) return Byte.toUnsignedInt((byte) SaLayout.FAMILY8.get(mseg));
    else return Short.toUnsignedInt((short) SaLayout.FAMILY16.get(mseg));
  }
  ...
  private static OptionalInt readTotalLengthField(MemorySegment mseg) {
    if (!NativeABI.isBsdAbi()) return OptionalInt.empty();
  ...
  public String toString() {
    return (
      "AF_%s[".formatted(familyConstant().orElse(null)) +
